...
def run_demo(batch_size, available_memory_proportion):
    ...
    try:
        ...
        if "popart::Session::prepareDevice: Poplar compilation" in err.stdout.decode(
            "utf-8"
        ):
            return True, -1
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch-size-min", type=int, default=2, help="Batch size minimum (inclusive)"
    )
    parser.add_argument(
        "--batch-size-max",
        type=int,
    ...
    parser.add_argument(
        "--batch-size-step", type=int, default=2, help="Batch size step size"
    )
    parser.add_argument(
    ...
    amp_values = np.arange(
        opts.available_memory_min, opts.available_memory_max, opts.available_memory_step
    )
    ...
    sweep_config_str = (
        f"bs-{opts.batch_size_min}-{opts.batch_size_step}-{opts.batch_size_max}"
    )
