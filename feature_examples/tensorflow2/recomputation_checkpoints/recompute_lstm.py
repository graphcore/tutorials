...
def make_split_layer_model(args):
    ...
    def split_lstm(seq_len, checkpoints, inputs, num_units):
        ...
        for i in range(checkpoints + 1):
            ...
            seq_slice = tf.keras.layers.Lambda(lambda x, b=0, e=0: x[:, b:e, :],
                                               arguments={'b': begin, 'e': end}
                                               )(inputs)
    ...
    with ipu.keras.PipelineStage(0):
        x = split_lstm(seq_len=args.seq_len,
...
def main():
    ...
    parser.add_argument("--checkpoints", type=int, default=7,
                        help="How many recomputation checkpoints to use.")
    parser.add_argument("--seq-len", type=int, default=2048,
                        help="The length of the LSTM sequence.")
    args = parser.parse_args()
    if args.seq_len % (args.checkpoints + 1) != 0:
        print(f"A sequence of length {args.seq_len} can't be evenly divided by "
