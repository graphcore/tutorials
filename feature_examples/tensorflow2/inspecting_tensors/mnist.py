...
def create_dataset():
    ...
    train_ds = tf.data.Dataset.from_tensor_slices(
        (x_train, y_train)).shuffle(len(x_train)).batch(32, drop_remainder=True)
...
def main():
    ...
    parser.add_argument('--no-pipelining', action="store_true",
                        help="Do not use pipelining")
    ...
    parser.add_argument('--use-gradient-accumulation', action='store_true',
    ...
    with strategy.scope():
        ...
        if not args.no_pipelining:
            if args.model_type == "Model":
                model = create_pipeline_model(multi_activations_outfeed_queue, gradient_accumulation_steps_per_replica)
                callbacks += [multi_layer_outfeed_callback]
            elif args.model_type == "Sequential":
                model = create_pipeline_sequential_model(multi_activations_outfeed_queue, gradient_accumulation_steps_per_replica)
                callbacks += [multi_layer_outfeed_callback]
        else:
        ...
        model.compile(loss=keras.losses.SparseCategoricalCrossentropy(),
                      optimizer=OutfeedOptimizer(keras.optimizers.SGD(),
