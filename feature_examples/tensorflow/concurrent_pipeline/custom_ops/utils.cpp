...
GatherAttributes readJsonGatherAttributes(const std::string &attributes) {
  ...
  try {
    ...
    throw std::runtime_error(
        std::string("Error reading custom op's JSON attributes: ") + e.what() +
...
void checkGatherTensorDimensions(const GatherAttributes &attr,
  ...
  if (features.dim(0) != attr.feature_count) {
    throw std::logic_error(
        "Feature count in tensor input does not match JSON attributes.");
  }

  if (features.dim(1) != attr.feature_dim) {
    throw std::logic_error(
        "Feature dimension in tensor input does not match JSON attributes.");
  }

  if (intIndices.dim(0) != attr.output_count) {
    throw std::logic_error(
        "Indices/output count in tensor input does not match JSON attributes.");
  }
}
...
poplar::OptionFlags getPoplarOptionsFromString(const std::string &jsonOptions,
...
poplar::Type typeFromString(const std::string &dtype) {
  ...
  try {
    ...
    throw std::runtime_error("Conversion to Poplar type not supported for: " +
                             dtype);
  }
}

std::map<std::size_t, poplar::Interval>
getShardInfo(const poplar::Graph &graph) {
  ...
  for (auto i = 0u; i < numIpus; ++i) {
    ...
    info.insert(std::make_pair(
        i, poplar::Interval(nextShardStartTile, nextShardEndTile)));
...
std::vector<poplar::Graph> createIpuShards(poplar::Graph &graph) {
  ...
  std::size_t nextShardStartTile = 0;
  // std::cerr << "Target num IPUs (num shards): " <<
  // graph.getTarget().getNumIPUs() << "\n"; std::cerr << "Tiles per IPU: " <<
  ...
  for (auto s = 0u; s < graph.getTarget().getNumIPUs(); ++s) {
    auto nextShardEndTile =
        nextShardStartTile + graph.getTarget().getTilesPerIPU();
    shard.push_back(
        graph.createVirtualGraph(nextShardStartTile, nextShardEndTile));
    // std::cerr << "Created virtual graph for tiles: [" << nextShardStartTile
    // << ", " << nextShardEndTile << ")\n";
    nextShardStartTile = nextShardEndTile;
  }
  return shard;
...
inline poplar::Interval intersect(const poplar::Interval &a,
...
poplar::Interval getTileInterval(const poplar::Graph &g,
  ...
  if (mapping.empty()) {
    throw std::runtime_error(
        "Called getTileInterval() on tensor with no tile mapping.");
  }
...
std::set<std::size_t> getIPUMapping(const poplar::Graph &g,
                                    const poplar::Tensor &t) {
...
void printTileMapping(poplar::Graph &g, poplar::Tensor &t,
                      const std::string &id) {
  auto mapping = g.getTileMapping(t);
...
// number of IPUs).
std::vector<poplar::Interval> getShardIntervals(std::size_t numShards,
                                                std::size_t dimSize) {
...
/// Allocate the RHS (weights). The weights need to be explicitly sharded over
/// all IPUs by columns (because this implementation is targeting tied embedding
/// projection matrices).
poplar::Tensor allocateShardedWeightMatrix(std::vector<poplar::Graph> &shards,
  ...
  for (auto c = 0u; c < intervals.size(); ++c) {
    ...
    auto tensor = poplin::createMatMulInputRHS(
        shard, type, {attr.rows_lhs, attr.cols_lhs},
        {attr.cols_lhs, rhsColumns}, prefix + "/rhs", matmulOpts,
        getPlanningCache());
...
getShardedTensorChunks(std::vector<poplar::Graph> &shards, std::size_t axis,
...
void checkShardedTensorMapping(
    poplar::Graph &graph, std::size_t numShards,
    const std::vector<poplar::Tensor> &shardedTensor) {
  // Check slices are correct:
  if (shardedTensor.size() != numShards) {
    throw std::runtime_error(
        "No. of weight matrix slices does not match no. of shards.");
  }
...
void checkOkForSharding(std::size_t numShards, Attributes &attr) {
  ...
  if (attr.cols_rhs % numShards) {
    throw std::runtime_error(
        "No. of RHS columns must be multiple of no. of shards (" +
        std::to_string(numShards) + ")");
  }
}

void checkExpectedSharding(poplar::Graph &g, poplar::Tensor t,
                           std::set<std::size_t> &&expectedIpus) {
...
void checkExpectedSharding(poplar::Graph &g, poplar::Tensor t,
                           std::initializer_list<std::size_t> &&expectedIpus) {
...
void addToDiagonal(poplar::Graph &g, poplar::Tensor m, poplar::Tensor v,
  ...
  if (m.shape().size() != 2 || m.dim(0) != m.dim(1)) {
    throw std::logic_error(
        "First tensor in addToDiagonal must be a square matrix.");
  }
  if (v.shape().size() != 1) {
  ...
  for (auto i = 0u; i < length; ++i) {
    diagonalElements[i] =
        m.slice({i, i}, {i + 1, i + 1}); // Using indexing gives segfault
  }
...
adjustIndicesAndCreateMasks(poplar::Graph &graph,
  ...
  for (auto s = 0u; s < shards.size(); ++s) {
    ...
    auto partitionSize =
        shards[s].addConstant(unsignedIndices.elementType(), {}, size);
    auto partitionStart =
        shards[s].addConstant(unsignedIndices.elementType(), {}, s * size);
    ...
    popops::subInPlace(shards[s], unsignedIndices[s], partitionStart, prog,
    ...
    auto mask = popops::map(shards[s], betweenExpr, operands, prog,
