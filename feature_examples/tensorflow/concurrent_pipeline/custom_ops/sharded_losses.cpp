...
poplar::program::Program sharded_log_softmax(
  ...
  for (auto s = 0u; s < shards.size(); ++s) {
    ...
    auto max =
        popops::reduce(shards[s], logits, {1},
  ...
  for (auto s = 0u; s < shards.size(); ++s) {
    auto logits = shardedLogits[s];
    auto max =
        maxSharded[s].squeeze({0}).expand({1}).broadcast(logits.dim(1), 1);
    ...
    auto numerators =
        popops::exp(shards[s], translated, prog, debug_prefix + "partial_exp");
    auto partialDenominator = popops::reduce(
        shards[s], numerators, numerators.elementType(), {1},
  ...
  auto denom = gcl::allReduceWithinReplica(
      graph, interIpuDenom, gcl::CollectiveOperator::ADD, prog,
      debug_prefix + "all_reduce_denominator");
  ...
  auto logsum =
      popops::map(graph,
                  popops::expr::Cast(popops::expr::Log(popops::expr::_1),
                                     input.elementType()),
                  {denom}, prog, "log_denominator")
          .expand({2})
          .broadcast(translatedInput.dim(2), 2);
  popops::subInPlace(graph, translatedInput, logsum, prog,
                     debug_prefix + "subtract_logsum");
...
void sharded_log_softmax_grad_metadata(
    std::vector<std::int64_t> &allocating_indices,
    std::vector<std::int64_t> &replica_identical_output_indices,
    std::map<std::int64_t, std::int64_t> &input_to_output_tensor_aliasing,
    bool &is_elementwise, bool &is_stateless, bool &is_hashable,
  ...
  is_stateless = true;
  input_to_output_tensor_aliasing.insert(std::make_pair(0, 0));
}

poplar::program::Program
sharded_log_softmax_grad(poplar::Graph &graph, int input_grad_index,
  ...
  //         where dL/dy = gradients[0] and (the row vector) s =
  //         exp(fwd_outputs[0])
  ...
  //
  // TODO: This fusion/optimisation means that the matrix formed by broadcasting
  // is only sharded in the columns of the RHS matrix which is what we want,
  // however dL/dx (gradients[0]) is also sharded by columns and we do not want
  // to exchange dL/dx between IPUs if it is large (e.g. as it will be in
  // embedding projections for large vocab language models). To avoid this we
  // can note that if the operation following log-softmax is cross-entropy then
  // dL/dx has only one non-zero (per grad) so we can ensure that single element
  // is already on the correct IPU using a custom cross-entropy layer.
  poplar::program::Sequence prog;

  // Recover the softmax activations s = softmax(x) (fwd_outputs[0] = y =
  // log(s)):
  auto s = popops::exp(graph, fwd_outputs[0], prog,
                       debug_prefix + "/recover_softmax_acts");
  auto dType = s.elementType();

  // We want to compute dL/dx = dL/dy * (I - S) so first form S which is just a
  // broadcast of the activations:
  auto S = s.expand({1}).broadcast(s.dim(1), 1);
  auto dLdy = gradients[0].expand({1});

  // Instead of adding identity matrix to the diagonal of -S to form (I - S) we
  // can transform the equation to do everything in one accumulating matmul as
  // follows: dL/dx = dL/dy * (I - S) = dL/dy - (dL/dy * S)
  auto dLdx = dLdy; // dL/dx can just be an alias for the incoming gradient.

  // Do the batch of VJPs and squeeze the grad vectors into a matrix to return
  // them:
  ...
  poplin::matMulGroupedAcc(graph, dLdy, -1.f, dLdy, S, prog,
                           debug_prefix + "/batch_vjp", matmul_opts,
                           getPlanningCache());
...
poplar::program::Program sharded_take_last(
  ...
  if (inputs[1].rank() != 3) {
    std::stringstream ss;
    ss << "Indices in op '" << debug_prefix
       << "' are expected to be a sharded copy over "
          "all IPUs and therefore should have rank 3. (Try using "
          "sharded.copy_to_all())"
       << "\n";
  ...
  for (auto s = 0u; s < shards.size(); ++s) {
    ...
    for (auto r = 0u; r < shardInput.dim(0); ++r) {
      ...
      auto scalar = popops::multiSlice(
          shards[s], sliceableRow, rowIndex, {0}, {1}, prog, {}, {},
          debug_prefix + "/take_along_row_" + rowSuffix + "_shard_" +
              shardSuffixStr);
  ...
  auto result = popops::reduce(graph, partials, partials.elementType(), {0},
                               ...
                               debug_prefix + "reduce_partial_denom");
  outputs.push_back(result);

  return prog;
}

void sharded_take_last_grad_metadata(
    std::vector<std::int64_t> &allocating_indices,
    std::vector<std::int64_t> &replica_identical_output_indices,
    std::map<std::int64_t, std::int64_t> &input_to_output_tensor_aliasing,
    bool &is_elementwise, bool &is_stateless, bool &is_hashable,
...
poplar::program::Program
sharded_take_last_grad(poplar::Graph &graph, int input_grad_index,
                       const std::vector<poplar::Tensor> &gradients,
                       const std::vector<poplar::Tensor> &fwd_inputs,
                       const std::vector<poplar::Tensor> &fwd_outputs,
                       std::vector<poplar::Tensor> &outputs,
  ...
  // TODO: This is very inefficient for large "vocab" dimension as it doesn't
  // respect sharding at all (but if you are concerned with performance you
  // should be using the fused BWD pass that comes from using
  // `sharded_log_softmax_cross_entropy()`).
  auto prog = poplar::program::Sequence();
  auto unsignedIndices = popops::cast(graph, indices, poplar::UNSIGNED_INT,
                                      prog, debug_prefix + "/cast_indices");
  popops::fill(graph, dLdx, prog, 0.f, debug_prefix + "/init_zero");
  for (auto i = 0u; i < x.dim(0); ++i) {
    popops::multiUpdateAdd(
        graph, dLdx[i].expand({1}), gradient[i].expand({0, 0}),
        unsignedIndices[i].expand({0}), scale, {0}, {1}, prog, {}, {},
...
void sharded_log_softmax_cross_entropy_grad_metadata(
    std::vector<std::int64_t> &allocating_indices,
    std::vector<std::int64_t> &replica_identical_output_indices,
    std::map<std::int64_t, std::int64_t> &input_to_output_tensor_aliasing,
    bool &is_elementwise, bool &is_stateless, bool &is_hashable,
...
poplar::program::Program sharded_log_softmax_cross_entropy(
  ...
  poplar::program::Sequence prog;
  prog.add(sharded_log_softmax(graph, inputs, logSoftmaxOutputs, attributes,
                               debug_prefix + "/log_softmax"));
  popops::negInPlace(graph, logSoftmaxOutputs[0], prog,
                     debug_prefix + "/neg_log_softmax");

  // Pass the label indices forward to take_last:
  logSoftmaxOutputs.push_back(inputs[1]);

  poplar::program::Sequence takeLastProg;
  takeLastProg.add(sharded_take_last(graph, logSoftmaxOutputs, outputs,
                                     attributes, debug_prefix + "/take_last"));
  outputs.push_back(logSoftmaxOutputs[0]); // We also stash -ve log-softmax for
                                           // the backwards pass.
...
poplar::program::Program sharded_log_softmax_cross_entropy_grad(
  ...
  auto s = popops::exp(graph, log_softmax_output, prog,
                       debug_prefix + "/recover_softmax_acts");

  // TODO: Not sure this happens in parallel? (Check it happens in parallel on
  // each shard):
  for (auto i = 0u; i < log_softmax_output.dim(0); ++i) {
    auto scale = gradients[0][i];
    popops::mulInPlace(graph, s[i], scale, prog,
                       debug_prefix + "/scale_softmax_acts");
  }

  // Finally we need to scatter-subtract the incoming gradient from what we have
  ...
  prog.add(copyToAll(graph, gradient, gradientShardedCopies,
                     debug_prefix + "/broadcast_grad_to_shards"));
  ...
  for (auto c = 0u; c < shards.size(); ++c) {
    ...
    for (auto i = 0u; i < activation.dim(0); ++i) {
      ...
      popops::multiUpdateAdd(
          shards[c], updateable.expand({1}), grad[i].expand({0, 0}),
          adjustedIndices[c][i].expand({0}), minusOne, {0}, {1}, prog, {}, {},
          debug_prefix + "/scatter_gradients_" + shardSuffixStr + "_" +
              std::to_string(i));
      // Copy result back:
