...
sharded_matmul(poplar::Graph &graph, const std::vector<poplar::Tensor> &inputs,
  ...
  if (input.dim(0) != shard.size() || input.rank() != 3 ||
      weights.rank() != 2) {
    throw std::logic_error("LHS input must be a matrix broadcast across IPUs "
                           "(rank 3) and RHS must be a matrix (rank 2).");
  }
  ...
  for (auto s = 0u; s < shard.size(); ++s) {
    auto result =
        poplin::matMul(shard[s], input[s], shardedWeights[s], prog,
                       debug_prefix + "/fwd_matmul_shard" + std::to_string(s),
...
poplar::Tensor sharded_matmul_allocator(poplar::Graph &graph,
  ...
  if (operand == 0) {
    ...
    return poplin::createMatMulInputLHS(
        shards[0], type, {attr.rows_lhs, attr.cols_lhs},
        {attr.cols_lhs, weightColumnsPerShard}, prefix + "/lhs", matmul_opts,
        getPlanningCache());
  }
  ...
  if (operand == 1) {
    ...
    std::cerr << prefix
              << ": allocating sharded weights with shape: " << weights.shape()
...
void sharded_matmul_grad_metadata(
    std::vector<std::int64_t> &allocating_indices,
    std::vector<std::int64_t> &replica_identical_output_indices,
    std::map<std::int64_t, std::int64_t> &input_to_output_tensor_aliasing,
    bool &is_elementwise, bool &is_stateless, bool &is_hashable,
...
poplar::program::Program
sharded_matmul_grad(poplar::Graph &graph, int input_grad_index,
  ...
  // We checked the input is copied to every shard above and the incoming
  // gradient is sharded across IPUs by columns so all computation is local to
  // each shard:
  std::vector<poplar::Tensor> sharded_dLdW;
  for (auto s = 0u; s < shard.size(); ++s) {
    auto chunkOfdLdW =
        poplin::matMul(shard[s], inputTransposed[s], shardedGradOfOutput[s],
                       prog, debug_prefix + "/dLdW_shard_" + std::to_string(s),
                       matmul_opts, getPlanningCache());
  ...
  for (auto s = 0u; s < shard.size(); ++s) {
    auto result = poplin::matMul(
        shard[s], shardedGradOfOutput[s], shardedWeights[s].transpose(), prog,
        debug_prefix + "/dLdI_shard" + std::to_string(s), matmul_opts,
        getPlanningCache());
    partialResults.push_back(result.expand(
        {0})); // expand tensors so they can be concatted and reduced
  }
  ...
  checkExpectedSharding(graph, partialGradOfLossWrtInput,
                        getIpuSet(shard.size()));
