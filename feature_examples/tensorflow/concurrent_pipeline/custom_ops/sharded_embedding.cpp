...
poplar::program::Program debug(poplar::Graph &graph,
  ...
  std::cerr << "FWD Op: " << debug_prefix << "\n  Tensor '"
            << inputs[0].getDebugStr() << "' shape: " << inputs[0].shape()
            << " sharding: " << getIPUMapping(graph, inputs[0]) << "\n";
...
debug_grad(poplar::Graph &graph, int input_grad_index,
  ...
  std::cerr << "BWD Op: " << debug_prefix << "\n  Tensor '"
            << gradients[0].getDebugStr() << "' shape: " << gradients[0].shape()
            << " sharding: " << getIPUMapping(graph, gradients[0]) << "\n";
...
poplar::program::Program embedding(poplar::Graph &graph,
                                   const std::vector<poplar::Tensor> &inputs,
                                   std::vector<poplar::Tensor> &outputs,
                                   const std::string &attributes,
                                   const std::string &debug_prefix) {
  if (inputs.size() != 2) {
    throw poputil::poplibs_error(
        "Embedding requires 2 inputs (features, indices).");
  }
  ...
  std::tie(adjustedIndices, masks) =
      adjustIndicesAndCreateMasks(graph, partitionSizes, inputs[1], program,
                                  debug_prefix + "/adjust_indices");
  ...
  for (auto s = 0u; s < shards.size(); ++s) {
    ...
    popops::SlicePlan slicePlan = popops::embedding::plan(
        shards[s], features.elementType(), shardFeatures.dim(0),
        shardFeatures.dim(1), {shardIndices.dim(1), 1}, opts);
    auto shardSlices =
        popops::multiSlice(shards[s], shardFeatures, shardIndices, {0}, {1},
                           program, slicePlan, {});

    // Masks are vectors so we need to broadcast the mask to match the feature
    // dimension:
  ...
  std::vector<poplar::Tensor> resultsNotOnShard0(results.begin() + 1,
                                                 results.end());
  auto partials = poplar::concat(resultsNotOnShard0, 0);
  poplar::program::Sequence prog;
  popops::reduceWithOutput(graph, partials, results[0], {0},
                           popops::ReduceParams(popops::Operation::ADD, true),
                           program, debug_prefix + "/reduce_partial_slices");
  // Reduce with output produces an unnecessary extra dimension compared to
  // regular reduce and we also want to remove the extra dimension produced by
  // slicing (NOTE: in the backwards we need to undo the squeeze of the
  // dimension introduced by slice but not the one due to reduce):
  outputs.push_back(results[0].squeeze({0, 2}));
  return program;
}

poplar::Tensor embedding_allocator(poplar::Graph &graph, std::uint32_t operand,
  ...
  if (attr.feature_count % shards.size()) {
    std::stringstream ss;
    ss << prefix
       << ": Features/vocab size does not divide equally into the number of "
          "shards."
       << "\n";
  ...
  if (operand == 0) {
    ...
    std::cerr << "Allocating sliceable weight tensor for operand "
              << std::to_string(operand) << " across " << shards.size()
              << " shards with " << rowsPerShard << " rows per shard\n";

    std::vector<poplar::Tensor> shardedWeights;

    for (auto s = 0u; s < shards.size(); ++s) {
      popops::SlicePlan slicePlan = popops::embedding::plan(
          shards[s], type, rowsPerShard, attr.feature_dim,
          {attr.output_count, 1}, opts);
      auto w = popops::createSliceableTensor(shards[s], type,
  ...
  if (operand == 1) {
    ...
    std::cerr << "Allocating index tensor for operand "
              << std::to_string(operand) << "\n";

    // We allocate indices entirely on shard 0: it is up to user code to
    // broadcast it to other shards.
    popops::SlicePlan slicePlan =
        popops::embedding::plan(shards[0], type, rowsPerShard, attr.feature_dim,
                                {attr.output_count, 1}, opts);

    if (type != poplar::INT) {
      throw std::logic_error("Expected INT type for operand: " +
                             std::to_string(operand));
    }

    auto indices = popops::createIndicesTensor(
        graph, {rowsPerShard}, attr.output_count, slicePlan, {});
    // We need to reinterpret the type as IPU XLA does not support unsigned
    // ints:
    return indices.reinterpret(poplar::INT);
  }

  throw std::logic_error("Allocator called for invalid operand: " +
                         std::to_string(operand));
...
poplar::program::Program
embedding_grad(poplar::Graph &graph, int input_grad_index,
               const std::vector<poplar::Tensor> &gradients,
               const std::vector<poplar::Tensor> &fwd_inputs,
               const std::vector<poplar::Tensor> &fwd_outputs,
               std::vector<poplar::Tensor> &outputs,
               const std::string &attributes, const std::string &debug_prefix) {
  std::cerr << "Building custom gather gradient op index " +
                   std::to_string(input_grad_index) +
                   " with attributes: " + attributes + "\n";

  if (input_grad_index != 0) {
    throw std::logic_error("Only the features have a gradient. "
                           "Note: You must pass 'separate_gradients=True' to "
                           "precompiled_user_op() ");
  }
  ...
  std::vector<poplar::Tensor> masks;
  std::tie(adjustedIndices, masks) =
      adjustIndicesAndCreateMasks(graph, partitionSizes, fwd_inputs[1], program,
                                  debug_prefix + "/adjust_indices");
  ...
  program.add(copyToAll(graph, grad, gradBroadcast,
                        debug_prefix + "/broadcast_grad_to_all_shards"));
  ...
  for (auto s = 0u; s < shards.size(); ++s) {
    ...
    auto shardIndices = adjustedIndices[s];
    // program.add(poplar::program::PrintTensor("Adjusted Indices shard" +
    // shardSuffixStr, adjustedIndices[s]));
    popops::SlicePlan slicePlan = popops::embedding::plan(
        shards[s], features.elementType(), shardGatherGrad.dim(0),
        shardGatherGrad.dim(1), {shardIndices.dim(1), 1}, opts);
    auto scale = shards[s].addConstant(shardGatherGrad.elementType(), {},
                                       attr.gradient_scale);
    shards[s].setTileMapping(scale, getFirstTile(shards[s], shardGatherGrad));
    popops::multiUpdateAdd(shards[s], shardGatherGrad, gradBroadcast[s],
                           shardIndices, scale, {0}, {1}, program, slicePlan,
                           {}, debug_prefix + "/gather_grad_update_add");
  }
...
poplar::Tensor allocate_tied_embedding_allocator(
    ...
    const std::string &attributes, const std::string &prefix) {
...
poplar::program::Program
allocate_tied_embedding_grad(poplar::Graph &graph, int input_grad_index,
                             const std::vector<poplar::Tensor> &gradients,
                             const std::vector<poplar::Tensor> &fwd_inputs,
                             const std::vector<poplar::Tensor> &fwd_outputs,
                             std::vector<poplar::Tensor> &outputs,
