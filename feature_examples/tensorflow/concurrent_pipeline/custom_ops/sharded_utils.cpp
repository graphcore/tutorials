...
/// partial gradients on each shard so the backwards pass for this op is simply
/// to reduce them back onto the original shard.
///
/// Note: If the nature of the downstream op makes it more efficient to return
/// the total grad on one shard, then that op should do this broadcast in its
/// own forward pass (i.e. this op is not relevant for that use case as it
/// stands).
...
poplar::Tensor copy_to_all_allocator(poplar::Graph &graph,
...
poplar::program::Program copyToAll(poplar::Graph &graph, poplar::Tensor input,
  ...
  if (inputIpus.size() != 1) {
    std::stringstream ss;
    ss << "In op '" << debug_prefix << "' the input tensor '"
       << input.getDebugStr() << "' is already sharded on IPUs " << inputIpus;
...
poplar::program::Program copy_to_all(poplar::Graph &graph,
...
void copy_to_all_grad_metadata(
    std::vector<std::int64_t> &allocating_indices,
    std::vector<std::int64_t> &replica_identical_output_indices,
    std::map<std::int64_t, std::int64_t> &input_to_output_tensor_aliasing,
    bool &is_elementwise, bool &is_stateless, bool &is_hashable,
...
copy_to_all_grad(poplar::Graph &graph, int input_grad_index,
  ...
  popops::reduceWithOutput(graph, gradients[0], gradOfLossWrtInput, {0},
                           popops::ReduceParams(popops::Operation::ADD, false),
                           prog, debug_prefix + "/dLdI_accumulate_partials");
  ...
  checkExpectedSharding(graph, gradOfLossWrtInput,
                        getIPUMapping(graph, fwd_inputs[0]));
  return prog;
}
