...
def parse_args():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=8, help="The (micro-)batch size."
    )
    parser.add_argument(
        "--repeat-count",
        type=int,
    ...
    parser.add_argument(
        "--epochs", type=float, default=30, help="Total number of epochs to train over."
    )
    parser.add_argument(
        "--learning-rate",
        type=float,
...
def custom_dense(hidden_size, namescope, input, kernel_initializer):
    matmul_opts = {"availableMemoryProportion": "0.8", "partialsType": "half"}
    with tf.variable_scope(namescope, reuse=tf.AUTO_REUSE, use_resource=True):
        weights = tf.get_variable(
            "weights", dtype=input.dtype, trainable=True, initializer=kernel_initializer
        )
...
def classifier_stage(lr, input, labels):
    ...
    logits = layers.Dense(
        hidden_size,
        name="stage2_dense",
        kernel_initializer=ClosureInitializer(init_func),
    )(input)
