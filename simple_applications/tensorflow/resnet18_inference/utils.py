...
def _fc(x, out_dim, input_q=None, output_q=None, name="fc"):
    ...
    with tf.variable_scope(name):
        ...
        w = tf.get_variable(
            ...
            x.dtype,
            initializer=tf.random_normal_initializer(stddev=np.sqrt(1.0 / out_dim)),
        )
        b = tf.get_variable(
            "biases", [out_dim], x.dtype, initializer=tf.constant_initializer(0.0)
        )
...
def _add_split_loss(w, input_q, output_q):
    ...
    assert (
        input_q.get_shape().as_list()[0] == output_q.get_shape().as_list()[0]
    )  # ngroups
    ...
    if input_q not in tf.get_collection("OVERLAP_LOSS_WEIGHTS"):
        ...
        print(f"\t\tAdd overlap & split loss for {input_q.name}")
    ...
    if output_q not in tf.get_collection("OVERLAP_LOSS_WEIGHTS"):
        print(f"\t\tAdd overlap & split loss for {output_q.name}")
    ...
    for i in range(ngroups):
        if len(w.get_shape()) == 4:
            ...
            wg_row = tf.matmul(
                tf.matmul(tf.diag(1 - input_q[i, :]), w), tf.diag(output_q[i, :])
            )
            wg_row_l2 = tf.reduce_sum(tf.sqrt(tf.reduce_sum(wg_row * wg_row, 1)))
            wg_col = tf.matmul(
                tf.matmul(tf.diag(input_q[i, :]), w), tf.diag(1 - output_q[i, :])
            )
    ...
    tf.histogram_summary(f"{scope_name}/weights", w)
    print(
        f"\t\tAdd split loss for {tf.get_variable_scope().name}"
        f"({in_dim}x{out_dim}, {ngroups} groups)"
    )
...
def _bn(x, is_train, name="bn"):
    with tf.variable_scope(name):
        ...
        mu = tf.get_variable(
            "mu", batch_mean.get_shape(), tf.float16, initializer=tf.zeros_initializer()
        )
