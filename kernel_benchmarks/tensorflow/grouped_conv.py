...
def conv2d_basic(
    x, ksize, stride, filters_in, filters_out, index, groups=1, name="conv"
):
    with tf.variable_scope(name, use_resource=True):
        W = tf.get_variable(
            "conv2d/kernel" + str(index),
            shape=[ksize, ksize, filters_in / groups, filters_out],
            dtype=x.dtype,
            trainable=True,
...
def inputs(opts, index):
    ...
    return {
        "inputs": tf.broadcast_to(
            value, [opts.batch_size, opts.input_size, opts.input_size, opts.filter_in]
        ),
    }
...
def add_args(parser):
    ...
    parser.add_argument(
        "--filter-in", default=64, type=int, help="Number of filters in the input"
    )
    parser.add_argument(
        "--filter-out", default=64, type=int, help="Number of filters in the output"
    )
    ...
    parser.add_argument("--input-size", default=56, type=int, help="Input size")
    parser.add_argument(
        "--block-repeats", default=10, type=int, help="Number of convolution blocks"
    )
    parser.add_argument(
        "--group-dim",
        default=1,
...
if __name__ == "__main__":
    ...
    print(
        f"CNN layer {('Training' if options.train else 'Inference')} Synthetic benchmark.\n"
        f" Batch size {options.batch_size}.\n"
        f" Batches per Step {options.batches_per_step}.\n"
        f" Steps {options.steps}.\n"
        f" Input size {options.input_size}.\n"
        f" Group dimension {options.group_dim}.\n"
        f" Output filters {options.filter_out}.\n"
        f" Block repeat {options.block_repeats}.\n"
    )
