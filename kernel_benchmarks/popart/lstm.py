...
def add_args(parser):
    ...
    parser.add_argument(
        "--timesteps", type=int, default=1, help="Set number of recurrent steps."
    )
    parser.add_argument(
        "--hidden-size",
        type=int,
...
if __name__ == "__main__":
    ...
    print(
        f"PopART LSTM {mode_names[opts.mode]} Synthetic benchmark.\n"
        f" Batch size {opts.batch_size}.\n"
        f" Batches per Step {opts.batches_per_step}.\n"
        f" Steps {opts.steps}.\n"
        f" Hidden size {opts.hidden_size}.\n"
        f" Timesteps {opts.timesteps}.\n"
    )
