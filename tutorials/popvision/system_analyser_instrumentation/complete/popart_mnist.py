...
def train(opts):
    ...
    for i in range(opts.epochs):
        ...
        with pvti.Tracepoint(mnistPvtiChannel, f"Epoch:{i}"):
            ...
            # Training
            with pvti.Tracepoint(mnistPvtiChannel, f"Training"):
                if i > 0:
                ...
                for step, (data, labels) in enumerate(training_set):
                    ...
                    results = np.argmax(
                        training.anchors[output].reshape(
                            [training_set.inputs_per_step, 10]
                        ),
                        1,
                    )
                    num_correct = np.sum(
                        results == labels.reshape([training_set.inputs_per_step])
                    )
                    training_accuracy_series.add(
                        num_correct / training_set.inputs_per_step
                    )
            ...
            # Evaluation
            with pvti.Tracepoint(mnistPvtiChannel, f"Evaluation"):
                ...
                for step, (data, labels) in enumerate(test_set):
                    ...
                    validation_accuracy_series.add(
                        num_correct / test_set.inputs_per_step
                    )
